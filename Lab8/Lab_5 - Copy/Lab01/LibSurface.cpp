...
//Алгоритм основан на принципе того, что объекты, которые находятся дальше от наблюдателя, должны быть отображены позади тех, что ближе.
//Суть заключается в том, чтобы сначала отрисовывать 
//все видимые части объектов, которые находятся в передней части сцены, а затем постепенно «закрашивать» те части, которые находятся дальше.
// 1.Представление объектов:
// 2.Вычисление глубины:
// 3. Сортировка граней по глубине:
// 4. Рендеринг в порядке глубины:
